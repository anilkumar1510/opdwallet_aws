...
import { findPortalToken } from '@/lib/session';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // (another portal on localhost sets the same name), so pick this portal's —
    // forwarding a member's session would report the wrong user back to the UI.
    const portalToken = findPortalToken(cookie);
    const opdSessionCookie = portalToken ? `opd_session=${portalToken}` : null;
