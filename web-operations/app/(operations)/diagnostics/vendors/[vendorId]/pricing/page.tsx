...
import { ChevronLeftIcon, PlusIcon, PencilIcon } from '@heroicons/react/24/outline'
...
export default function DiagnosticVendorPricingPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Fetch vendor
      const vendorRes = await apiFetch('/api/admin/diagnostics/vendors')
      if (vendorRes.ok) {
      ...
      // Fetch pricing
      const pricingRes = await apiFetch(`/api/admin/diagnostics/vendors/${vendorId}/pricing`)
      if (pricingRes.ok) {
        const pricingData = await pricingRes.json()
        setPricingList(pricingData.data || [])
      }

      // Fetch all diagnostic services
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingPricing
        ? `/api/admin/diagnostics/vendors/${vendorId}/pricing/${formData.serviceId}`
        : `/api/admin/diagnostics/vendors/${vendorId}/pricing`
      ...
      <div className="bg-white shadow-sm">
        <div className="px-4 py-4 flex items-center">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-semibold text-gray-900">Diagnostic Vendor Pricing</h1>
