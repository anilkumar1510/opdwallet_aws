...
export default function VisionServicesPage() {
  const [clinics, setClinics] = useState<Clinic[]>([])
  const [loading, setLoading] = useState(true)
  const [expandedClinic, setExpandedClinic] = useState<string | null>(null)
  const [services, setServices] = useState<Record<string, Service[]>>({})
  const [slots, setSlots] = useState<Record<string, VisionSlot[]>>({})
  ...
  const fetchClinics = useCallback(async () => {
    try {
      console.log('[Vision Services] Fetching all clinics...')
      setLoading(true)
      const response = await apiFetch('/api/ops/vision-services/clinics')

      if (response.ok) {
        const result = await response.json()
        console.log('[Vision Services] Clinics fetched:', result)
        setClinics(result.data || [])
      } else {
        console.error('[Vision Services] Failed to fetch clinics: HTTP', response.status)
        setClinics([])
      }
    } catch (error) {
      console.error('[Vision Services] Error fetching clinics:', error)
      setClinics([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchServicesForClinic = useCallback(async (clinicId: string) => {
    try {
      console.log(`[Vision Services] Fetching services for clinic ${clinicId}...`)
      setLoadingServices(clinicId)
      const response = await apiFetch(`/api/ops/vision-services/clinics/${clinicId}/services`)

      if (response.ok) {
        const result = await response.json()
        console.log(`[Vision Services] Services fetched for ${clinicId}:`, result)
        setServices(prev => ({ ...prev, [clinicId]: result.data || [] }))
      } else {
        console.error(`[Vision Services] Failed to fetch services for ${clinicId}: HTTP`, response.status)
      }
    } catch (error) {
      console.error(`[Vision Services] Error fetching services for ${clinicId}:`, error)
    } finally {
  ...
  const fetchSlotsForClinic = useCallback(async (clinicId: string) => {
    try {
      console.log(`[Vision Slots] Fetching slots for clinic ${clinicId}...`)
      setLoadingSlots(clinicId)
      const response = await apiFetch(`/api/ops/vision-services/clinics/${clinicId}/slots`)

      if (response.ok) {
        const result = await response.json()
        console.log(`[Vision Slots] Slots fetched for ${clinicId}:`, result)
        setSlots(prev => ({ ...prev, [clinicId]: result.data || [] }))
      } else {
        console.error(`[Vision Slots] Failed to fetch slots for ${clinicId}: HTTP`, response.status)
      }
    } catch (error) {
      console.error(`[Vision Slots] Error fetching slots for ${clinicId}:`, error)
    } finally {
      setLoadingSlots(null)
    }
  }, [])

  // Toggle clinic-level vision services enabled/disabled
  const toggleClinicVisionServices = useCallback(async (
    clinicId: string,
    ...
    try {
      ...
      console.log(`[Vision Services] Toggling vision services for clinic ${clinicId} to ${newState}`)
      ...
      if (response.ok) {
        const result = await response.json()
        console.log(`[Vision Services] Clinic vision services toggled successfully:`, result)

        setClinics(prev => prev.map(c =>
          c.clinicId === clinicId
            ? { ...c, visionServicesEnabled: newState, hasEnabledServices: newState ? c.hasEnabledServices : false, enabledServicesCount: newState ? c.enabledServicesCount : 0 }
            : c
        ))
        ...
        toast.success(`Vision services ${newState ? 'enabled' : 'disabled'} for clinic`)
        fetchClinics()
      } else {
        const error = await response.json()
        console.error(`[Vision Services] Failed to toggle clinic vision services:`, error)
        toast.error(`Failed to toggle vision services: ${error.message || 'Unknown error'}`)
      }
    } catch (error) {
      console.error(`[Vision Services] Error toggling clinic vision services:`, error)
      toast.error('Failed to toggle vision services. Please try again.')
    } finally {
  ...
  const toggleService = useCallback(async (
    ...
    try {
      ...
      console.log(`[Vision Services] Toggling service ${serviceCode} for clinic ${clinicId} to ${newState}`)

      setUpdatingService(key)
      const response = await apiFetch(
        `/api/ops/vision-services/clinics/${clinicId}/services/${serviceCode}/toggle`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ isEnabled: newState }),
      ...
      if (response.ok) {
        const result = await response.json()
        console.log(`[Vision Services] Service toggled successfully:`, result)
        ...
        const error = await response.json()
        console.error(`[Vision Services] Failed to toggle service:`, error)
        toast.error(`Failed to toggle service: ${error.message || 'Unknown error'}`)
      }
    } catch (error) {
      console.error(`[Vision Services] Error toggling service:`, error)
  ...
  const openSlotModal = (clinic: Clinic) => {
    console.log(`[Vision Slots] Opening slot modal for clinic ${clinic.clinicId}`)
  ...
  const createSlots = async () => {
    ...
    try {
      console.log(`[Vision Slots] Creating slots for clinic ${selectedClinicForSlot.clinicId}`, slotFormData)

      const response = await apiFetch(
        `/api/ops/vision-services/clinics/${selectedClinicForSlot.clinicId}/slots`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const deleteSlot = async (slotId: string, clinicId: string) => {
    ...
    try {
      console.log(`[Vision Slots] Deleting slot ${slotId}`)

      const response = await apiFetch(
        `/api/ops/vision-services/slots/${slotId}`,
        {
          method: 'DELETE',
        }
      )
      ...
      if (response.ok) {
        const result = await response.json()
        console.log(`[Vision Slots] Slot deleted successfully:`, result)
        toast.success('Slot deleted successfully')
        fetchSlotsForClinic(clinicId)
      } else {
        const error = await response.json()
        console.error(`[Vision Slots] Failed to delete slot:`, error)
        toast.error(`Failed to delete slot: ${error.message || 'Unknown error'}`)
      }
    } catch (error) {
      console.error(`[Vision Slots] Error deleting slot:`, error)
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Vision Services Management</h1>
        <p className="mt-2 text-gray-600">Manage vision services and time slots for clinics</p>
      </div>
              ...
              >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col items-center">
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            toggleClinicVisionServices(clinic.clinicId, clinic.visionServicesEnabled)
                          }}
                        ...
                        >
                          <span
                            className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${
                              clinic.visionServicesEnabled ? 'translate-x-7' : 'translate-x-1'
                            }`}
                          />
                        </button>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-200 p-6 bg-gray-50">
                    ...
                    {/* Services Section */}
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900 mb-4">Vision Services</h3>
                      {loadingServices === clinic.clinicId ? (
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
                          <p className="text-gray-600">No vision services found</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {clinicServices.map((service) => {
                            const isUpdating = updatingService === `${clinic.clinicId}-${service.code}`
                            const isDisabled = !clinic.visionServicesEnabled
                              ...
                              >
                                <div className="flex items-start justify-between">
                                  ...
                                  <button
                                    ...
                                    title={isDisabled ? 'Enable clinic-level vision services first' : (service.isEnabledForClinic ? 'Disable service' : 'Enable service')}
                                  >
                                    <span
