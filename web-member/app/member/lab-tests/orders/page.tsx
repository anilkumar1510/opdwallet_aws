...
export default function LabOrdersPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#f7f7fc' }}>
      <PageHeader
        title="My Lab Orders"
        subtitle="Track your lab test orders"
        onBack={() => router.back()}
      />
      ...
      <div className="max-w-[480px] mx-auto lg:max-w-4xl px-4 lg:px-6 py-6 lg:py-8 space-y-4 lg:space-y-5">
        {orders.length === 0 ? (
          <EmptyState
            icon={BeakerIcon}
            title="No orders yet"
            message="You haven't booked any lab tests yet"
            ctaText="Book Lab Tests"
            onCtaClick={() => router.push('/member/lab-tests')}
          />
        ) : (
          orders.map((order) => (
            <DetailCard
            ...
            >
              ...
              <div className="mt-4 lg:mt-5 flex flex-col lg:flex-row space-y-2 lg:space-y-0 lg:space-x-3">
                <div className="flex-1">
                  <CTAButton
                    onClick={() => router.push(`/member/lab-tests/orders/${order.orderId}`)}
                    variant="primary"
                    fullWidth
                  >
                    View Details
