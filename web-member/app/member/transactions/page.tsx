...
export default function TransactionsPage() {
  ...
  // Client-side search filter (backend handles other filters)
  ...
  const setDateRange = (range: string) => {
    ...
    switch (range) {
      ...
      case '3months':
        ...
        date90.setMonth(date90.getMonth() - 3)
  ...
  const openPopup = (popupName: string, buttonEl: HTMLElement) => {
    ...
    switch (popupName) {
      ...
      case 'sortBy':
        ...
        break
      case 'includeReversed':
        setTempIncludeReversed(includeReversed)
        break
    }
  }

  // Confirm popup - apply temp state to actual filter state
  ...
  // Cancel popup - discard temp state
  const cancelPopup = () => {
    setActivePopup(null)
    // Temp state is discarded, will be re-initialized on next open
  }

  // Helper to toggle temp types
  ...
  // Helper to toggle temp categories
  ...
  // Helper to toggle temp service types
    ...
    <div className="min-h-screen pb-24" style={{ backgroundColor: '#f7f7fc' }}>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <button
                ...
                className="px-3 py-2 text-xs font-semibold rounded-lg border-2 transition-colors hover:bg-blue-50"
          ...
          <div className="flex flex-col gap-3">
            <div>
              <label className="block text-xs font-medium mb-1.5 text-gray-600">From</label>
              <input
                type="date"
                value={tempDateFrom}
            ...
            <div>
              <label className="block text-xs font-medium mb-1.5 text-gray-600">To</label>
              <input
                type="date"
                value={tempDateTo}
      ...
      >
        <div className="flex flex-col gap-3">
          <div>
            <label className="block text-xs font-medium mb-1.5 text-gray-600">Min Amount</label>
            <input
              type="number"
              placeholder="₹0"
              value={tempMinAmount}
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5 text-gray-600">Max Amount</label>
            <input
              type="number"
              placeholder="₹10000"
              value={tempMaxAmount}
        ...
        <div className="flex flex-col gap-2 max-h-80 overflow-y-auto">
          {walletBalance?.categories.map((cat) => (
            ...
            >
              <input
                type="checkbox"
                checked={tempCategories.includes(cat.categoryCode)}
                onChange={() => toggleTempCategory(cat.categoryCode)}
                className="w-4 h-4 rounded border-2 cursor-pointer"
                style={{ borderColor: '#86ACD8', accentColor: '#0F5FDC' }}
              />
              <span className="text-sm font-medium" style={{ color: '#0E51A2' }}>
          ...
          {availableServiceTypes.map((st) => (
            ...
            >
              <input
                ...
                onChange={() => toggleTempServiceType(st)}
                className="w-4 h-4 rounded border-2 cursor-pointer"
                style={{ borderColor: '#86ACD8', accentColor: '#0F5FDC' }}
              />
              <span className="text-sm font-medium" style={{ color: '#0E51A2' }}>
      ...
      >
        <div className="flex flex-col gap-4">
          {/* Sort Field */}
          <div>
            <label className="block text-xs font-bold mb-2 uppercase tracking-wide" style={{ color: '#0E51A2' }}>
              Sort Field
            </label>
            <div className="flex flex-col gap-2">
              <label className="flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer transition-colors hover:bg-blue-50">
                ...
                />
                <span className="text-sm font-medium" style={{ color: '#0E51A2' }}>Date</span>
              </label>
              <label className="flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer transition-colors hover:bg-blue-50">
                <input
                  type="radio"
                  ...
                  style={{ accentColor: '#0F5FDC' }}
                />
                <span className="text-sm font-medium" style={{ color: '#0E51A2' }}>Amount</span>
              </label>
            </div>
          </div>
