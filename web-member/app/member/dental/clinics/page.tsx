...
export default function DentalClinicsPage() {
  ...
  const searchClinics = async (searchPincode: string) => {
    ...
    console.log('[DentalClinics] Searching clinics for pincode:', searchPincode, 'service:', serviceCode)
    setLoading(true)
    setError('')
    ...
    try {
      const response = await fetch(
        `/api/dental-bookings/clinics?serviceCode=${serviceCode}&pincode=${searchPincode}`,
        { credentials: 'include' }
      )

      if (response.ok) {
        const data = await response.json()
        console.log('[DentalClinics] Clinics found:', data.clinics?.length || 0)
        ...
        console.error('[DentalClinics] Failed to fetch clinics:', response.status)
  ...
  const handleSelectClinic = (clinicId: string) => {
    ...
    router.push(
      `/member/dental/select-patient?clinicId=${clinicId}&serviceCode=${serviceCode}`
    )
  }
