...
export default function DiagnosticMasterTestsPage() {
  ...
  const fetchTests = useCallback(async () => {
    try {
      ...
      const response = await apiFetch(`/api/admin/diagnostics/master-tests?${params}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingTest
        ? `/api/admin/diagnostics/master-tests/${editingTest.parameterId}`
        : '/api/admin/diagnostics/master-tests'
  ...
  const handleToggleStatus = async (test: MasterTestParameter) => {
    try {
      const response = await apiFetch(`/api/admin/diagnostics/master-tests/${test.parameterId}/status`, {
        method: 'PATCH',
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Diagnostic Master Tests</h1>
          <p className="text-sm text-gray-600 mt-1">
            Manage standardized diagnostic test names for vendor mapping
          </p>
        </div>
        <button
          onClick={() => {
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-auto">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-xl font-bold">
                {editingTest ? 'Edit Diagnostic Master Test' : 'Create Diagnostic Master Test'}
              </h3>
            </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., CTCHEST, MRI-BRAIN"
                      required
                    />
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="e.g., CT Scan - Chest, MRI - Brain"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Alternative names used by different vendors (comma-separated)
                  </p>
                </div>
              </div>
