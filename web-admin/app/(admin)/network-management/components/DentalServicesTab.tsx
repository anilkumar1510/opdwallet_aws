...
export default function DentalServicesTab() {
  ...
  useEffect(() => {
    ...
    if (filters.status !== '') {
      ...
      filtered = filtered.filter((clinic: Clinic) =>
        clinic.dentalServicesEnabled === statusBool
      )
    }
  ...
  const toggleDentalServices = async (clinicId: string, currentStatus: boolean) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Dental services ${action} successfully`)
        fetchClinics() // Refresh list
      } else {
        const error = await response.json()
        toast.error(error.message || 'Failed to toggle dental services')
      }
    } catch (error) {
      console.error('Failed to toggle dental services:', error)
      toast.error('Failed to toggle dental services')
    } finally {
      setToggling(null)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Dental Services Management</h3>
          <p className="text-sm text-gray-600 mt-1">
            Enable or disable dental services for clinics. Service pricing and slots are managed in Operations Portal.
          </p>
        </div>
      </div>
                  ...
                  clinics.map((clinic) => (
                    <tr key={clinic.clinicId}>
                      ...
                      <td>
                        <span
                          className={`badge ${
                            clinic.dentalServicesEnabled ? 'badge-success' : 'badge-error'
                          }`}
                        >
                      ...
                      <td>
                        <div className="flex justify-center">
                          <button
                            onClick={() => toggleDentalServices(clinic.clinicId, clinic.dentalServicesEnabled)}
