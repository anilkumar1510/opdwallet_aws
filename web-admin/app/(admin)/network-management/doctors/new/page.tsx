...
export default function NewDoctorPage() {
  ...
  const fetchSpecialties = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setSpecialties(data.filter((s: any) => s.isActive))
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[CreateDoctor] Response status:', response.status, response.ok)

      if (response.ok) {
        console.log('[CreateDoctor] Success! About to redirect...')
        toast.success('Doctor created successfully')
        console.log('[CreateDoctor] Calling router.replace with /network-management?tab=doctors')
        router.replace('/network-management?tab=doctors')
        console.log('[CreateDoctor] router.replace called')
      } else {
        console.log('[CreateDoctor] Response NOT ok, status:', response.status)
      ...
      console.error('[CreateDoctor] Exception caught:', error)
      ...
      console.log('[CreateDoctor] Finally block, setSaving(false)')
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => router.push('/network-management?tab=doctors')}
        ...
        <div className="card">
          ...
          {loadingClinics ? (
            ...
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
              ...
              <button
                type="button"
                onClick={() => router.push('/network-management/clinics/new?returnTab=doctors')}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, selectedClinicId: e.target.value })}
                className="input"
                required
              >
