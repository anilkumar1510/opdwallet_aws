...
interface VaccinationServiceSelectorProps {
  categoryId: string;
  categoryName: string;
  policyId: string;
  version: number;
  selectedServiceIds: string[];  // Array of serviceId values
  onSelectionChange: (serviceIds: string[]) => void;
  disabled?: boolean;
  isNew?: boolean;
}

export function VaccinationServiceSelector({
  categoryId,
  categoryName,
  policyId,
  version,
  ...
  disabled = false,
  isNew = false,
}: VaccinationServiceSelectorProps) {
  ...
  const fetchAvailableServices = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error('Failed to fetch available vaccination services');
      }
    } catch (error) {
  ...
  const handleSelectAll = async () => {
    ...
    const allServiceIds = filteredServices.map((s) => s.serviceId);
    onSelectionChange(allServiceIds);
    ...
    try {
      const response = await apiFetch(
        ...
        {
          method: 'PATCH',
          body: JSON.stringify({ serviceIds: allServiceIds }),
        }
      );
      ...
      if (!response.ok) {
        ...
        toast.success('All vaccination services selected');
      }
    } catch (error) {
  ...
  const handleClearAll = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.success('All vaccination services cleared');
      }
    } catch (error) {
  ...
  const filteredServices = services.filter((service) =>
    ...
    (service.vaccineType && service.vaccineType.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (service.manufacturer && service.manufacturer.toLowerCase().includes(searchQuery.toLowerCase()))
  );
  ...
  if (services.length === 0) {
    return (
      <div className="text-center py-8 text-gray-600">
        <p className="mb-2">No vaccination services available.</p>
        <p className="text-sm text-gray-500">
          Create vaccination services in the Vaccination Management section first.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex-1 relative mr-4">
          ...
          <input
            type="text"
            placeholder="Search vaccines by name, code, type, or manufacturer..."
            value={searchQuery}
      ...
      <div className="mb-3 text-sm text-gray-700">
        ...
        <span className="font-semibold">{totalCount}</span> vaccination services selected
      </div>

      {/* Info Message */}
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-xs text-blue-800">
          <strong>Note:</strong> Only selected vaccination services will be available to members with this policy.
          {selectedCount === 0 && ' No vaccination services are currently selected - members won\'t have access to any vaccines.'}
        </p>
      </div>

      {/* Vaccination Services List */}
        ...
        {filteredServices.map((service) => {
          ...
          return (
            <div
              ...
              `}
              onClick={() => !disabled && handleToggleService(service.serviceId, !isSelected)}
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  {service.dosesRequired && service.dosesRequired > 0 && (
              ...
              <Switch
                checked={isSelected}
                onCheckedChange={(checked) => handleToggleService(service.serviceId, checked)}
      ...
      {filteredServices.length === 0 && searchQuery && (
        <div className="text-center py-8 text-gray-500">
          No vaccination services found matching &quot;{searchQuery}&quot;
        </div>
      )}
    </div>
  );
