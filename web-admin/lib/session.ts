...
// must pick the admin one out of the pile rather than taking the first.
...
export function isAdminToken(token: string | undefined): boolean {
  return adminRoles.includes(getSessionRole(token) ?? '')
}

// Picks the admin session out of a raw Cookie header that may carry several
// opd_session values.
export function findAdminToken(cookieHeader: string | null): string | null {
  ...
  return tokens.find(isAdminToken) ?? null
}
