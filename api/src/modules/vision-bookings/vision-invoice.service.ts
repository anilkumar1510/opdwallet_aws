...
@Injectable()
export class VisionInvoiceService {
  private readonly uploadDir = join(process.cwd(), 'uploads', 'invoices', 'vision');
  ...
  /**
   * Generate PDF invoice for vision booking
   */
  async generateInvoice(
    booking: VisionBookingDocument,
  ): Promise<{ invoiceId: string; filePath: string; fileName: string }> {
    console.log('[VisionInvoice] Generating invoice for booking:', booking.bookingId);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc
          .fontSize(24)
          .font('Helvetica-Bold')
          .text('VISION INVOICE', { align: 'center' })
          .moveDown(0.5);
        ...
        doc
          ...
          .fillColor('#999999')
          .text(
            'This is a system-generated invoice for vision services. For queries, contact the clinic.',
            50,
            doc.page.height - 100,
        ...
        stream.on('finish', () => {
          console.log('[VisionInvoice] Invoice PDF generated successfully:', fileName);
          resolve({ invoiceId, filePath, fileName });
        });

        stream.on('error', (error) => {
          console.error('[VisionInvoice] Error generating invoice:', error);
          reject(error);
        });
      } catch (error) {
        console.error('[VisionInvoice] Exception during invoice generation:', error);
        reject(error);
      }
    });
  }
  ...
  private generateInvoiceId(bookingId: string): string {
    const random = Math.floor(Math.random() * 1000)
      .toString()
      .padStart(3, '0');
    return `INV-VIS-${bookingId}-${random}`;
  }
}
