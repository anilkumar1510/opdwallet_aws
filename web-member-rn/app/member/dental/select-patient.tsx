...
export default function SelectPatientPage() {
  ...
  useEffect(() => {
    ...
    if (!clinicId || !serviceCode) {
      console.error('[SelectPatient] Missing clinicId or serviceCode, redirecting to dental page');
      router.replace('/member/dental');
      return;
    }

    console.log('[SelectPatient] Query params:', { clinicId, serviceCode });
  ...
  const handleContinue = useCallback(() => {
    ...
    router.push(
      `/member/dental/select-slot?clinicId=${clinicId}&serviceCode=${serviceCode}&patientId=${selectedPatient.id}` as any
    );
        ...
        <View style={{ maxWidth: 480, marginHorizontal: 'auto', width: '100%' }}>
          ...
          {patients.length === 0 ? (
            <View style={{ paddingVertical: 48, alignItems: 'center', marginBottom: 24 }}>
              <Text style={{ fontSize: 16, fontWeight: '600', color: '#0E51A2', marginBottom: 8 }}>No Patients Available</Text>
              <Text style={{ fontSize: 14, color: '#6B7280', textAlign: 'center' }}>Please contact support if you believe this is an error.</Text>
            </View>
          ) : (
