...
export default function SelectPatientPage() {
  ...
  useEffect(() => {
    ...
    if (!vendorId || !serviceId) {
      console.error('[SelectPatient] Missing vendorId or serviceId, redirecting');
      router.replace('/member/vaccination');
      return;
    }

    console.log('[SelectPatient] Query params:', { vendorId, serviceId, serviceName, price, pincode });
  }, [vendorId, serviceId, serviceName, price, pincode, router]);
  ...
  useEffect(() => {
    ...
    if (viewingUserId) {
      ...
      if (matchedPatient) {
        console.log('[SelectPatient] Auto-selected patient from viewing context:', matchedPatient.name);
  ...
  const handleContinue = useCallback(() => {
    ...
    // Build URL with optional pincode (only include if valid)
    const validPincode = pincode && pincode !== 'undefined' && pincode !== 'null' && pincode.trim().length > 0;
    const pincodeParam = validPincode ? `&pincode=${pincode}` : '';

    router.push(
      `/member/vaccination/select-slot?vendorId=${vendorId}&vendorName=${encodeURIComponent(vendorName)}&serviceId=${serviceId}&serviceName=${encodeURIComponent(serviceName)}&serviceCode=${serviceCode}&price=${price}${pincodeParam}&patientId=${selectedPatient.id}&patientName=${encodeURIComponent(selectedPatient.name)}` as any
    );
  }, [selectedPatient, router, vendorId, vendorName, serviceId, serviceName, serviceCode, price, pincode]);
